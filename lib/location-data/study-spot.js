...
module.exports = class StudySpot {
    ...
    append(availableSeats){
        ...
        const recentupdates = this.updates.filter(u => (new Date(u.timestamp).getTime() / 60000 + 30) > (new Date().getTime() / 60000));
        
        // Walgelijke fix maar voor test purposes SORRY!!!! :(
        let seat_list = recentupdates.filter(function(update) {
        ...
        this.updates = this.updates.filter(u => (new Date(u.timestamp).getTime() / 60000 + 60) > (new Date().getTime() / 60000));
